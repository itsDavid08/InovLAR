...
export default function PedidosTV({ emergencias, normais, onResolver }) {
            ...
            <div style={{ flex: "0 0 34%", display: "flex", flexDirection: "column", gap: "clamp(12px,1.2vw,20px)", minHeight: 0 }}>
                ...
                {emg.pageItems.map((e) => (
                    <div key={e.id} style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column",
                        ...
                        <div style={{ font: "900 clamp(24px,2.3vw,44px) system-ui", color: "#7f1d1d" }}>{e.nome}</div>
                        <div style={{ font: "700 clamp(15px,1.4vw,26px) system-ui", color: "#991b1b" }}>🚪 {e.quarto}</div>
                        ...
                        <button onClick={() => onResolver(e.id)} title="Concluir emergência"
                            style={{ marginTop: 8, cursor: "pointer", border: "none", borderRadius: 999,
                                background: "#15803d", color: "#fff", font: "900 clamp(16px,1.5vw,26px) system-ui",
                                padding: "10px 28px" }}>✔ Concluir</button>
                    </div>
                ))}
                ...
                {fila.pageItems.map((r) => (
                    <div key={r.id} style={{ flex: 1, minHeight: 0, background: "#fff", border: "2px solid #e8edf4",
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                            <div title={r.nome} style={{ font: "900 clamp(24px,2.4vw,44px) system-ui", color: "#0f172a",
                                whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.nome}</div>
                            <div style={{ font: "700 clamp(13px,1.2vw,22px) system-ui", color: "#64748b" }}>🚪 {r.quarto}</div>
                            <div style={{ font: "600 clamp(15px,1.5vw,27px) system-ui", color: "#1e293b",
                        ...
                        <button onClick={() => onResolver(r.id)} title="Concluir pedido"
                            style={{ flex: "0 0 auto", cursor: "pointer", border: "none", borderRadius: 999,
                                ...
                                padding: "10px clamp(14px,1.4vw,22px)" }}>✔</button>
                    </div>
                ))}
                {normais.length === 0 && (
                    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", color: "#94a3b8",
