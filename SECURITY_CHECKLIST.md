...
- [x] **Cookie de sessão é um valor estático `"ok"` — não é uma sessão real**
  ...
  **Feito:** novo modelo `Server/models/StaffSession.js` (criado via `sync()` no `main.js`, sem migration — segue o padrão de `StaffAuth`/`TabelaLayout`/`TabelaPadrao`) + helper `Server/Util/sessions.js`. O cookie passa a guardar um token aleatório de 32 bytes; a BD guarda só o SHA-256 desse token + `expiraEm`. `requireStaff` (`middleware/auth.js`) passa a async e valida o token contra a BD; `logout` revoga a sessão (apaga a linha) — revogação real. Limpeza preguiçosa: linhas expiradas são apagadas quando validadas. `change` não revoga sessões (PIN partilhado de dispositivo). **Nota operacional:** dispositivos com o cookie antigo `"ok"` são deslogados uma vez após o deploy e reintroduzem o PIN.

- [x] **`COOKIE_SECRET` com fallback hardcoded no repositório público**
  ...
  **Feito:** `Server/config/auth.js` agora chama `require('dotenv').config()` (bug relacionado: o `require` deste ficheiro acontecia antes do dotenv carregar, via `main.js`, então um `.env` com `COOKIE_SECRET` nunca era lido) e recusa arrancar (`process.exit(1)`) sem `COOKIE_SECRET` quando `NODE_ENV=production`; em dev, avisa e usa um valor fixo só-para-dev. `.env.example`, `install.sh` e `install.ps1` passam a gerar/documentar `COOKIE_SECRET` automaticamente (mesmo padrão idempotente do `DB_PASS`).

- [x] **Cookie de sessão nunca usa a flag `Secure`**
  ...
  **Feito (parcialmente diferente do proposto):** `secure` passa a ler `process.env.COOKIE_SECURE === "true"` em vez de ligar a `NODE_ENV`. Motivo: a Pi em produção serve atualmente por HTTP simples, sem TLS configurado em lado nenhum do `install.sh` — ligar `secure` a `NODE_ENV=production` cegamente fazia o browser deixar de enviar o cookie e partia o login em produção imediatamente. `COOKIE_SECURE` fica documentado no `.env.example` como opt-in, a ativar só quando houver HTTPS (reverse proxy com certificado) à frente do servidor.
...
- [x] **`Server/.env.example` não lista `COOKIE_SECRET`**
  ...
  **Feito:** junto com os pontos críticos #2 e #3 acima — `.env.example` já documenta `COOKIE_SECRET` e `COOKIE_SECURE`.
