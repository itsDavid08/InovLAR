...
| Test Server | `cd Server && npm test` (Vitest — zod schemas, auth/validate middleware, board ownership, upload security) |
...
- **BUG — `colocarComEmpurrao` loses/overlaps buttons on push** (found 2026-07-23, documented, *not yet fixed*): it does not reserve the target footprint in `ocup` before relocating collided buttons, so a pushed button can land in the target's cells and then be overwritten by the final placement. Reachable via the **resize gesture over a neighbour** (`useGridResize.js` → `colocarComEmpurrao`): resizing a button onto an occupied neighbour makes the neighbour vanish (or overlap for multi-cell footprints). Captured as two `it.fails` (xfail) tests in `gridSpans.test.js` asserting the correct behaviour — when the algorithm is fixed, those turn red as a signal to drop the `.fails`.
